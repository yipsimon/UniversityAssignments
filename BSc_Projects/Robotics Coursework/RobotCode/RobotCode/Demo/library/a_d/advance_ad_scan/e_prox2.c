...
extern int e_ambient_ir2[8];						// ambient light measurement
extern int e_ambient_and_reflected_ir2[8];		// light when led is on

static int init_value_ir2[8] = {0,0,0,0,0,0,0,0};

/*! \brief To calibrate your ir sensor
 * \warning Call this function one time before calling e_get_calibrated_prox
 */
void e_calibrate_ir2()
{
	int i=0,j=0;
	int long t;
	int long tmp[8];
	...
	for (i=0;i<8;++i) {
		init_value_ir2[i]=(int)(tmp[i]/(j*1.0));
	}
}
 ...
 */
int e_get_prox2(unsigned int sensor_number)
{
	if (sensor_number > 7)
		return 0;
	else
		return e_ambient_ir2[sensor_number] - e_ambient_and_reflected_ir2[sensor_number];
}
 ...
 * \return The analogic value of the specified proxy sensor
 */
int e_get_calibrated_prox2(unsigned int sensor_number)
{
	int temp;
	if (sensor_number > 7)
		return 0;
	...
	{
		temp=(e_ambient_ir2[sensor_number] - e_ambient_and_reflected_ir2[sensor_number])
				- init_value_ir2[sensor_number];	
		if (temp>0)
			return temp;
		else
			return 0;
 ...
 * \return The analogic value of the specified proxy sensor
 */
int e_get_ambient_light2(unsigned int sensor_number)
{
	if (sensor_number > 7)
		return 0;
	else
		return e_ambient_ir2[sensor_number];
}
